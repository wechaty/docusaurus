...
```bash
npm install wechaty wechaty-redux ducks ts-node
```
...
Start by creating a new folder called `src`, and add a file `redux-bot.ts`. We will be writing the code here.
