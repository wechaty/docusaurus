...
```bash
https://github.com/wechaty/wechaty-getting-started.git
...
Create a new folder called `src` and add a file `room-bot.js`. You will be writing code here.
...
This function will help you find a room, join it, leave it and to change the topic of the room.
...
This function checks who can send an invitation and who can't. If the invitation is sent by the owner, then the person could join the room. Otherwise, the invitation will be cancelled and the person has to resend ding to the owner so that the owner can send him/her a new valid invitation.
...
This event gets the `user` and display the login status of the user. After that, it displays the `msg` saying `setting to manageDingRoom() after 3 seconds ... ` which means it will call `manageDingRoom` function after 3 seconds.
...
`room-join` event lets people to join a respective room.
...
This event removes people from room.
...
This event handles entry and exit in the room. If you send `ding` first time, then you will get an invitation but saying `ding` in a room leads to exiting the room. Similarly, if you say `dong` it will quit the room.
