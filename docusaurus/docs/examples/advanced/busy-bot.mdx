...
Busy bot sends auto response message for you when you are busy.
...
Create a new folder called `src` and add a file `busy-bot.js`. You will be writing code here.

Let's import required packages in `busy-bot.js` and initialise the bot by providing it a name and puppet to be used.
